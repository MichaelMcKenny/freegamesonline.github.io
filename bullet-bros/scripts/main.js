...
window.DOMElementHandler = class extends self.DOMHandler {
    ...
    _OnCreate(a) {
        ...
        this._autoAttach && (a = this.GetRuntimeInterface().GetAvailableHTMLIndex(e), d.SetActualHTMLIndex(a), this.GetRuntimeInterface().GetHTMLWrapElement(a).appendChild(c))
    }
    CreateElement(a, b) {
    ...
    _OnUpdatePosition(a) {
        if (this._autoAttach) {
            ...
            e !== b.GetActualHTMLIndex() && (c.remove(), d.GetHTMLWrapElement(e).appendChild(c), b.SetActualHTMLIndex(e), d._UpdateHTMLElementsZOrder());
            a = a.htmlZIndex;
            a !== b.GetHTMLZIndex() && (b.SetHTMLZIndex(a), d._UpdateHTMLElementsZOrder());
    ...
    _OnHTMLLayersChanged() {
        if (this._autoAttach)
            for (const b of this._elementMap.values()) {
                const c =
                    this.GetRuntimeInterface().GetAvailableHTMLIndex(b.GetWantHTMLIndex());
                var a = b.GetActualHTMLIndex(); - 1 !== c && -1 !== a && c !== a && (a = b.GetElement(), a.remove(), this.GetRuntimeInterface().GetHTMLWrapElement(c).appendChild(a), b.SetActualHTMLIndex(c))
            }
    }
...
/safari/i.test(navigator.userAgent) && /(chrome|chromium|edg\/|OPR\/|nwjs)/i.test(navigator.userAgent);
...
window.RuntimeInterface = class a {
    constructor(b) {
        ...
        this.IsAnyWebView2Wrapper() ? self.chrome.webview.addEventListener("message", c => this._OnWrapperMessage(c.data)) : "macos-wkwebview" === this._exportType && (self.C3WrapperOnMessage = c => this._OnWrapperMessage(c));
        ...
        this.AddRuntimeComponentMessageHandler("canvas",
            "update-size", c => this._OnUpdateCanvasSize(c));
        this.AddRuntimeComponentMessageHandler("canvas", "set-html-layer-count", c => this._OnSetHTMLLayerCount(c));
        ...
        this.AddRuntimeComponentMessageHandler("runtime",
            "send-wrapper-extension-message", c => this._OnSendWrapperExtensionMessage(c));
    ...
    Release() {
        ...
        for (const {
                canvas: b,
                htmlWrap: c
            } of this._canvasLayers) b.remove(),
            c.remove();
    ...
    GetHTMLWrapElement(b) {
        if (0 > b || b >= this._canvasLayers.length) throw new RangeError("invalid canvas layer");
    ...
    IsAnyWebView2Wrapper() {
        return this.IsWindowsWebView2() ||
            "xbox-uwp-webview2" === this._exportType
    }
    async _Init(b) {
        this._useWorker && !await CheckSupportsWorkerMode() && (this._useWorker = !1);
        "macos-wkwebview" === this._exportType ? this._SendWrapperMessage({
            type: "ready"
        }) : this.IsAnyWebView2Wrapper() && (this._SetupWebView2Polyfills(), this._wrapperComponentIds = (await this._InitWrapper()).registeredComponentIds);
        if ("playable-ad" === this._exportType) {
            ...
            for (let d = 0, e = b.engineScripts.length; d <
                e; ++d) {
        ...
        b.runtimeBaseUrl ? this._runtimeBaseUrl = b.runtimeBaseUrl : (c = location.origin,
            this._runtimeBaseUrl = ("null" === c ? "file:///" : c) + location.pathname, c = this._runtimeBaseUrl.lastIndexOf("/"), -1 !== c && (this._runtimeBaseUrl = this._runtimeBaseUrl.substr(0, c + 1)));
    ...
    _GetWorkerURL(b) {
        b = this._workerScriptURLs.hasOwnProperty(b) ? this._workerScriptURLs[b] : b.endsWith("/workermain.js") && this._workerScriptURLs.hasOwnProperty("workermain.js") ?
            this._workerScriptURLs["workermain.js"] : "playable-ad" === this._exportType && this._localFileBlobs.hasOwnProperty(b) ? this._localFileBlobs[b] : b;
    ...
    async _InitWorker(b, c) {
        ...
        if ("cordova" ===
            ...
            for (let k = 0, m = b.projectScripts.length; k < m; ++k) {
                const n = b.projectScripts[k],
                    p = n[0];
                if (p === b.mainProjectScript || "scriptsInEvents.js" === p || p.endsWith("/scriptsInEvents.js")) n[1] = await this._MaybeGetCordovaScriptURL(p)
            }
        ...
        self.c3_callFunction = (k, m) => this._runtimeDomHandler._InvokeFunctionFromJS(k, m);
    ...
    async _InitDOM(b,
        ...
        e = b.engineScripts.map(g => "string" === typeof g ? (new URL(g, this._runtimeBaseUrl)).toString() : g);
        if (Array.isArray(b.workerDependencyScripts)) {
            var f = [...b.workerDependencyScripts].map(g => g instanceof Blob ? URL.createObjectURL(g) : g);
            e.unshift(...f)
        }
        ...
        for (let [g, l] of h)
            if (l || (l = g), g === f) try {
                l = await this._MaybeGetCordovaScriptURL(l),
                    await AddScript(l), "preview" !== this._exportType || e[g] || this._ReportProjectMainScriptError(g, "main script did not run to completion")
            } catch (k) {
                this._ReportProjectMainScriptError(g, k)
            } else if ("scriptsInEvents.js" === g || g.endsWith("/scriptsInEvents.js")) l = await this._MaybeGetCordovaScriptURL(l), await AddScript(l);
    ...
    _OnUpdateCanvasSize(b) {
        if (!this.IsExportingToVideo()) {
            ...
            for (const {
                    canvas: h,
                    htmlWrap: g
                } of this._canvasLayers) h.style.width = c, h.style.height = d, h.style.marginLeft = e, h.style.marginTop = f, g.style.width = c, g.style.height = d, g.style.marginLeft = e, g.style.marginTop = f, this._isFirstSizeUpdate && (h.style.display = "", g.style.display = "");
            document.documentElement.style.setProperty("--construct-scale", b.displayScale);
    ...
    _OnSetHTMLLayerCount(b) {
        ...
        if (c < this._canvasLayers.length)
            for (; this._canvasLayers.length > c;) {
                ...
                m.remove();
                this._useWorker && !d ? this._pendingRemoveElements.push(k) : k.remove()
            } else if (c > this._canvasLayers.length)
                for (let k = 0, m = c - this._canvasLayers.length; k < m; ++k) c = document.createElement("canvas"), c.classList.add("c3overlay"), this._useWorker ? (d = c.transferControlToOffscreen(), g.push(d),
                    l.push(d)) : g.push(c), document.body.appendChild(c), d = document.createElement("div"), d.classList.add("c3htmlwrap", "c3overlay"), document.body.appendChild(d), c.style.width = e, c.style.height = f, c.style.marginLeft = h, c.style.marginTop = b, d.style.width = e, d.style.height = f, d.style.marginLeft = h, d.style.marginTop = b, this._runtimeDomHandler._AddDefaultCanvasEventHandlers(c), this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(d), this._canvasLayers.push({
                    canvas: c,
                    htmlWrap: d
                });
        for (const k of this._domHandlers) k instanceof
        window.DOMElementHandler && k._OnHTMLLayersChanged();
    ...
    _DoUpdateHTMLElementsZOrder() {
        ...
        for (; e < f; ++e) {
            const h = b[e];
            h.GetActualHTMLIndex() !== c && (this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c, b.slice(d, e)), c = h.GetActualHTMLIndex(), d = e)
        }
        d < e && this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c,
            b.slice(d, e))
    }
    _DoUpdateHTMLElementsZOrderOnHTMLLayer(b, c) {
        if (!(1 >= c.length || b >= this._canvasLayers.length)) {
            ...
            for (var e = Array.from(b.children).filter(l => d.has(l)), f = 0, h = Math.min(c.length, e.length); f < h && c[f] === e[f]; ++f);
    ...
    _OnEventFromRuntime(b) {
        ...
        if (b = runtimeEventHandlers.get(c))
            if (b = b.get(d)) {
                ...
                if (null === f) return h;
                h && h.then ? h.then(g => this._PostResultToRuntime(f, !0, g)).catch(g => {
    ...
    async _MaybeGetCordovaScriptURL(b) {
        return "cordova" === this._exportType && (b.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(b)) ? (b.startsWith(this._runtimeBaseUrl) && (b = b.substr(this._runtimeBaseUrl.length)),
            b = await this.CordovaFetchLocalFileAsArrayBuffer(b), b = new Blob([b], {
                type: "application/javascript"
    ...
    async _CordovaDoFetchLocalFileAsAsArrayBuffer(b,
        c, d) {
        try {
    ...
    _OnWrapperMessage(b) {
        ...
        else if ("exited-fullscreen" === b) a._SetWrapperIsFullscreenFlag(!1), this._runtimeDomHandler._OnFullscreenChange();
        else if ("object" === typeof b) {
            const c = b.type;
            "wrapper-init-response" === c ? (this._wrapperInitResolve(b), this._wrapperInitResolve = null) : "extension-message" === c ? this.PostToRuntimeComponent("runtime",
                "wrapper-extension-message", b) : console.warn("Unknown wrapper message: ", b)
    ...
    _SendWrapperMessage(b) {
        this.IsAnyWebView2Wrapper() ? window.chrome.webview.postMessage(JSON.stringify(b)) : "macos-wkwebview" === this._exportType && window.webkit.messageHandlers.C3Wrapper.postMessage(JSON.stringify(b))
    }
    ...
    _InitWrapper() {
        return this.IsAnyWebView2Wrapper() ? new Promise(b => {
    ...
    async _ConvertDataUrisToBlobs() {
        const b = [];
        for (const [c, d] of Object.entries(this._localFileBlobs)) b.push(this._ConvertDataUriToBlobs(c,
            d));
        await Promise.all(b)
    }
    ...
    _BinaryStringToBlob(b, c) {
        ...
        let e = d >> 2,
            ...
            g, l;
        for (l = g = 0; g < e; ++g) h[g] =
            b.charCodeAt(l++) | b.charCodeAt(l++) << 8 | b.charCodeAt(l++) << 16 | b.charCodeAt(l++) << 24;
...
async function BlobToSvgImage(a, b, c) {
    ...
    if (e.hasAttribute("width") && e.hasAttribute("height")) {
        ...
        if (!f.includes("%") && !h.includes("%")) return await BlobToImage(a)
    }
...
function PreventDefaultOnHTMLWrap(a) {
    a.target.tagName && a.target.classList.contains("c3htmlwrap") && a.preventDefault()
}
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(class extends self.DOMHandler {
    constructor(a) {
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "invoke-download", c => this._OnInvokeDownload(c));
        a.AddRuntimeComponentMessageHandler("runtime", "load-webfonts", c => this._OnLoadWebFonts(c));
        a.AddRuntimeComponentMessageHandler("runtime",
            "raster-svg-image", c => this._OnRasterSvgImage(c));
        a.AddRuntimeComponentMessageHandler("runtime", "get-svg-image-size", c => this._OnGetSvgImageSize(c));
        a.AddRuntimeComponentMessageHandler("runtime", "set-target-orientation", c => this._OnSetTargetOrientation(c));
        a.AddRuntimeComponentMessageHandler("runtime", "register-sw", () => this._OnRegisterSW());
        a.AddRuntimeComponentMessageHandler("runtime", "post-to-debugger", c => this._OnPostToDebugger(c));
        a.AddRuntimeComponentMessageHandler("runtime", "go-to-script", c => this._OnPostToDebugger(c));
        a.AddRuntimeComponentMessageHandler("runtime", "before-start-ticking", () => this._OnBeforeStartTicking());
        a.AddRuntimeComponentMessageHandler("runtime", "debug-highlight", c => this._OnDebugHighlight(c));
        ...
        a.AddRuntimeComponentMessageHandler("runtime", "add-stylesheet", c => this._OnAddStylesheet(c));
        a.AddRuntimeComponentMessageHandler("runtime", "script-create-worker", c => this._OnScriptCreateWorker(c));
        a.AddRuntimeComponentMessageHandler("runtime", "alert", c => this._OnAlert(c));
        a.AddRuntimeComponentMessageHandler("runtime", "screen-reader-text", c => this._OnScreenReaderTextEvent(c));
        a.AddRuntimeComponentMessageHandler("runtime", "hide-cordova-splash", () => this._OnHideCordovaSplash());
        a.AddRuntimeComponentMessageHandler("runtime", "set-exporting-to-video", c => this._SetExportingToVideo(c));
        a.AddRuntimeComponentMessageHandler("runtime",
            "export-to-video-progress", c => this._OnExportVideoProgress(c));
        a.AddRuntimeComponentMessageHandler("runtime", "exported-to-video", c => this._OnExportedToVideo(c));
        a.AddRuntimeComponentMessageHandler("runtime", "exported-to-image-sequence", c => this._OnExportedToImageSequence(c));
        ...
        window.addEventListener("fullscreenerror", c => this._OnFullscreenError(c));
        window.addEventListener("webkitfullscreenerror", c => this._OnFullscreenError(c));
        window.addEventListener("mozfullscreenerror", c => this._OnFullscreenError(c));
        if (a.IsiOSWebView()) {
            if (window.visualViewport) {
                ...
                window.visualViewport.addEventListener("resize", () => {
                    ...
                    d > c && (document.scrollingElement.scrollTop = 0, document.scrollingElement.scrollLeft = 0);
                    c = d
                })
    ...
    Attach() {
        ...
        window.addEventListener("mousedown", b => this._OnMouseEvent("mousedown", b, DISPATCH_SCRIPT_ONLY));
        window.addEventListener("mousemove", b => this._OnMouseEvent("mousemove", b, DISPATCH_SCRIPT_ONLY));
        window.addEventListener("mouseup", b => this._OnMouseEvent("mouseup", b, DISPATCH_SCRIPT_ONLY));
        window.addEventListener("dblclick", b => this._OnMouseEvent("dblclick", b, DISPATCH_RUNTIME_AND_SCRIPT));
        window.addEventListener("wheel", b => this._OnMouseWheelEvent("wheel", b, DISPATCH_RUNTIME_AND_SCRIPT));
        window.addEventListener("pointerdown", b => {
            this._HandlePointerDownFocus(b);
            this._OnPointerEvent("pointerdown", b)
        });
        this._iRuntime.UsesWorker() && "undefined" !== typeof window.onpointerrawupdate && self === self.top ? window.addEventListener("pointerrawupdate",
            b => this._OnPointerRawUpdate(b)) : window.addEventListener("pointermove", b => this._OnPointerEvent("pointermove", b));
    ...
    _OnAndroidVirtualKeyboardChange(a, b) {
        ...
        if (0 < b) {
            ...
            c && (c = c.getBoundingClientRect(), a = (c.top + c.bottom) / 2 - (a - b) / 2, a > b && (a = b), 0 > a && (a = 0), 0 < a && (document.body.style.position = "absolute", document.body.style.overflow = "visible", document.body.style.transform = `translateY(${-a}px)`, this._vkTranslateYOffset = a))
        }
    }
    ...
    _OnVisibilityChange(a) {
        this._pageVisibilityIsHidden !== a && ((this._pageVisibilityIsHidden = a) ? this._iRuntime._CancelAnimationFrame() : this._iRuntime._RequestAnimationFrame(), this.PostToRuntime("visibilitychange", {
            hidden: a
        }), !a && this._iRuntime.IsiOSWebView() && (a = () => {
            document.scrollingElement.scrollTop = 0;
            document.scrollingElement.scrollLeft =
                0
        }, setTimeout(a, 50), setTimeout(a, 100), setTimeout(a, 250), setTimeout(a, 500)))
    }
    _OnKeyEvent(a, b) {
        "Backspace" === b.key && PreventDefaultOnCanvasOrDoc(b);
        if (this._iRuntime.IsAnyWebView2Wrapper()) {
            var c = ["r", "p", "f", "g", "u"];
            (["F3", "F5", "F7"].includes(b.key) || b.ctrlKey && c.includes(b.key)) && b.preventDefault()
        }
        this._isExportToVideo || (c = KEY_CODE_ALIASES.get(b.code) || b.code, this._PostToRuntimeMaybeSync(a, {
            code: c,
            key: b.key,
            which: b.which,
            repeat: b.repeat,
    ...
    _OnPointerEvent(a, b) {
        if (!this._isExportToVideo) {
            ...
            "mouse" === b.pointerType && (this._mousePointerLastButtons = b.buttons)
        }
    }
    ...
    _AttachDeviceOrientationEvent() {
        this._attachedDeviceOrientationEvent || (this._attachedDeviceOrientationEvent = !0, window.addEventListener("deviceorientation", a => this._OnDeviceOrientation(a)), window.addEventListener("deviceorientationabsolute",
            a => this._OnDeviceOrientationAbsolute(a)))
    }
    ...
    async _OnRasterSvgImage(a) {
        ...
        a = await self.C3_RasterSvgImageBlob(a.blob,
            a.imageWidth, a.imageHeight, a.surfaceWidth, a.surfaceHeight);
    ...
    _OnDebugHighlight(a) {
        if (a.show) {
            this._debugHighlightElem ||
                (this._debugHighlightElem = document.createElement("div"), this._debugHighlightElem.id = "inspectOutline", document.body.appendChild(this._debugHighlightElem));
    ...
    _OnScreenReaderTextEvent(a) {
        var b = a.type;
        "create" === b ? (b = document.createElement("p"), b.id = "c3-sr-" + a.id, b.textContent = a.text, this._screenReaderTextWrap.appendChild(b)) : "update" === b ? (b = document.getElementById("c3-sr-" +
            a.id)) ? b.textContent = a.text : console.warn(`[Construct] Missing screen reader text with id ${a.id}`) : "release" === b ? (b = document.getElementById("c3-sr-" + a.id)) ? b.remove() : console.warn(`[Construct] Missing screen reader text with id ${a.id}`) : console.warn(`[Construct] Unknown screen reader text update '${b}'`)
    }
...
"use strict";
window.C3_Is_Supported && (window.c3_runtimeInterface = new self.RuntimeInterface({
    useWorker: !1,
...
self.AudioDOMHandler = class extends self.DOMHandler {
    ...
    async _Play(a) {
        ...
        try {
            this._lastAudioInstance = await this._GetAudioInstance(b, c, d, f, e), k ? (this._lastAudioInstance.SetPannerEnabled(!0), this._lastAudioInstance.SetPan(k.x, k.y, k.z, k.angle, k.innerAngle, k.outerAngle, k.outerGain), k.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(k.uid)) :
                "number" === typeof m && 0 !== m ? (this._lastAudioInstance.SetStereoPannerEnabled(!0), this._lastAudioInstance.SetStereoPan(m)) : (this._lastAudioInstance.SetPannerEnabled(!1), this._lastAudioInstance.SetStereoPannerEnabled(!1)), this._lastAudioInstance.Play(h, g, l, n)
        } catch (p) {
    ...
    _SetEffectParam(a) {
        ...
        const c = a.index,
            ...
            f = a.ramp;
        a = a.time;
        for (const h of b) b = this._effects.get(h.toLowerCase()), !b || 0 > c || c >= b.length || b[c].SetParam(d, e, f, a);
        this._PostUpdatedFxState()
    }
    _RemoveEffects(a) {
        a = a.tags;
...
self.C3AudioInstance = class {
    ...
    SetPanXYZA(a, b, c, d) {
        if (this._isPannerEnabled) {
            ...
            if (f[0] !== h || f[1] !== d || 0 !== f[2]) f[0] = h, f[1] = d, f[2] = 0, this._pannerNode.setOrientation(...f)
        }
    }
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    Pause() {
        this._isPaused || this._isStopped || this.HasEnded() || (this._resumePosition = this.GetPlaybackTime(), this._isLooping && (this._resumePosition %=
            this.GetDuration()), this._isPaused = !0, this._bufferSource.stop(0))
    }
    Resume() {
        !this._isPaused || this._isStopped || this.HasEnded() || (this._ReleaseBufferSource(), this._bufferSource = this.GetAudioContext().createBufferSource(), this._bufferSource.buffer = this._buffer.GetAudioBuffer(), this._bufferSource.connect(this._gainNode), this._activeSource = this._bufferSource, this._bufferSource.onended = this._onended_handler, this._bufferSource.loop = this._isLooping, this._UpdateVolume(), this._UpdatePlaybackRate(), this._bufferSource.start(0,
            this._resumePosition), this._playStartTime = this.GetCurrentTime(), this._playFromSeekPos = this._resumePosition, this._isPaused = !1)
    }
    GetOutputVolume() {
    ...
    _UpdatePlaybackRate() {
        ...
        this._isTimescaled && (a *= this._audioDomHandler.GetTimeScale());
        this._bufferSource &&
            (this._bufferSource.playbackRate.value = a)
    }
    Seek(a) {
    ...
    SetSuspended(a) {
        a ? this.IsPlaying() ? (this._resumeMe = !0, this._resumePosition = this.GetPlaybackTime(), this._isLooping && (this._resumePosition %= this.GetDuration()), this._bufferSource.stop(0)) : this._resumeMe = !1 : this._resumeMe && (this._ReleaseBufferSource(), this._bufferSource = this.GetAudioContext().createBufferSource(),
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    async _Init() {
        ...
        try {
            ...
            a = !1, console.warn("[Construct] Error checking storage persisted state: ", b)
        }
        return {
            isPersistent: a
        }
    ...
    async _OnRequestPersistent() {
        try {
            ...
            return console.error("[Construct] Error requesting persistent storage: ", a), {
                isOk: !1
            }
        }
    }
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    _OnClose() {
        navigator.app && navigator.app.exitApp ? navigator.app.exitApp() :
            navigator.device && navigator.device.exitApp ? navigator.device.exitApp() : window.close()
    }
    _OnSetFocus(a) {
        a = a.isFocus;
    ...
    _OnLockOrientation(a) {
        ...
        if (screen.orientation && screen.orientation.lock) screen.orientation.lock(a).catch(b => console.warn("[Construct] Failed to lock orientation: ", b));
            ...
            screen.lockOrientation ?
                b = screen.lockOrientation(a) : screen.webkitLockOrientation ? b = screen.webkitLockOrientation(a) : screen.mozLockOrientation ? b = screen.mozLockOrientation(a) : screen.msLockOrientation && (b = screen.msLockOrientation(a));
            b || console.warn("[Construct] Failed to lock orientation")
        } catch (b) {
            console.warn("[Construct] Failed to lock orientation: ", b)
        }
    ...
    _OnNavigate(a) {
        ...
        if ("back" === b) navigator.app && navigator.app.backHistory ? navigator.app.backHistory() : window.history.back();
            ...
            const c = a.target;
            a = a.exportType;
            self.cordova && self.cordova.InAppBrowser ? self.cordova.InAppBrowser.open(b,
                "_system") : "preview" === a || this._iRuntime.IsAnyWebView2Wrapper() ? window.open(b, "_blank") : this._isConstructArcade || (2 === c ? window.top.location = b : 1 === c ? window.parent.location = b : window.location = b)
    ...
    _OnRequestFullscreen(a) {
        if (this._iRuntime.IsAnyWebView2Wrapper() || "macos-wkwebview" === this._exportType) self.RuntimeInterface._SetWrapperIsFullscreenFlag(!0), this._iRuntime._SendWrapperMessage({
            type: "set-fullscreen",
            fullscreen: !0
        });
        else {
...
self.RuntimeInterface.AddDOMHandlerClass(class extends self.DOMHandler {
    ...
    NotifyGameplayStart() {
        this._pokiSDKLoaded && (this._finishedLoadingSent || console.log("Don't forget to send the Loading Finished Notification, or configure the Poki Plugin to manage it automatically."),
            this._gameplayActive = !0, PokiSDK.gameplayStart())
    }
    NotifyGameplayStop() {
